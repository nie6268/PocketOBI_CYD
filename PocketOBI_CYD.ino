...
struct BatteryData {
  ...
  uint8_t overloadRawNib;   // raw low-5-bits value behind overloadPct, before x5 - could
  uint8_t overdischargeRawNib; // just as plausibly be an event count, see drawBatteryHealth
...
bool readStaticInfo() {
  ...
  if (gotMsg) {
    ...
    bat.overloadRawNib = ol & 0x1F;
    bat.overloadPct = (ol & 0xE0) ? (uint8_t)(bat.overloadRawNib * 5) : 0;
    // Over-discharge: msg[24], stored INVERTED in the high nibble, step 5.33 %.
    uint8_t odNib = (uint8_t)(~payload[24]) >> 4;
    bat.overdischargeRawNib = odNib;
    ...
    bat.overloadRawNib = 0;
    bat.overdischargeRawNib = 0;
    bat.healthEstPct = 0;
    bat.mfgYear = 0;
    bat.mfgMonth = 0;
    bat.mfgDay = 0;
...
void dumpDecoded() {
  ...
  Serial.printf("odEvents=%u olEvents=%u extValid=%d healthEst=%u%% odThr=%u%%(raw%u) olThr=%u%%(raw%u)\n",
                bat.odEventCount, bat.olEventCount, bat.extValid, bat.healthEstPct,
                bat.overdischargePct, bat.overdischargeRawNib, bat.overloadPct, bat.overloadRawNib);
...
void drawBatteryHealth() {
  ...
  for (int i = 0; i < cellCount; i++) { if (bat.cell[i] > 0.1f && bat.cell[i] < mn) mn = bat.cell[i];
                                if (bat.cell[i] > mx) mx = bat.cell[i]; }
  _ry = y + 46;   // back to the original offset (no separate threshold row anymore)
  ...
  // The raw protection threshold (bat.od/overloadPct) is always available (decoded from the
  // basic frame, no D4 needed) and is appended in parens - this is what the older PocketOBI
  // UI showed on its own; now it rides along with the newer event-count history.
  // "thr%%/rawN": the %% is our x5.33 guess at a protection threshold; rawN is the
  // untouched nibble it came from, which could just as plausibly BE an event count
  // (see chat: a rawN of 10 is a very ordinary occurrence tally, not just a threshold
  // step) - shown side by side since which reading is correct is genuinely unresolved.
  if (!bat.extValid) snprintf(buf, sizeof(buf), "- (%u%%/%u)", bat.overdischargePct, bat.overdischargeRawNib);
  else if (bat.odEventCount == 0) snprintf(buf, sizeof(buf), "none (%u%%/%u)", bat.overdischargePct, bat.overdischargeRawNib);
  else snprintf(buf, sizeof(buf), "%u (%u%%) %u%%/%u", bat.odEventCount, bat.odWearPct, bat.overdischargePct, bat.overdischargeRawNib);
  kvRow(tr(S_OVERDISCHARGE), buf, !bat.extValid ? COL_MUTED : (bat.odEventCount ? COL_TEXT : COL_GREEN));
  if (!bat.extValid) snprintf(buf, sizeof(buf), "- (%u%%/%u)", bat.overloadPct, bat.overloadRawNib);
  else if (bat.olEventCount == 0) snprintf(buf, sizeof(buf), "none (%u%%/%u)", bat.overloadPct, bat.overloadRawNib);
  else snprintf(buf, sizeof(buf), "%u (%u%%) %u%%/%u", bat.olEventCount, bat.olWearPct, bat.overloadPct, bat.overloadRawNib);
