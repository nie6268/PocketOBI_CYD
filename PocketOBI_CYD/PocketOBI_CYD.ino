 ...
 *    (Adafruit_GFX itself stays a dependency too, purely for its bundled
 *    Fonts/*.h smooth-font headers the v2.1.0 UI uses).
...
#include <TFT_eSPI.h>
#include <Adafruit_GFX.h>               // kept ONLY for the bundled GFX font headers below
...
void setup() {
  ...
  // protocol uses 9600 baud. The CYD has a real UART-to-USB chip
  // (CH340/CP2102), unlike the ESP32-C3's native USB CDC where the baud
  // value was cosmetic - it must match exactly here or bytes come through
  // garbled. Re-enable COMM_DEBUG? Set your Serial Monitor to 9600 too.
  Serial.begin(9600);
  delay(1000);
